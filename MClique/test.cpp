...
struct Maxclique {
	const vector<vector<bool> > e; int pk, level; const float Tlimit;
	...
	Maxclique(const vector<vector<bool> > conn, const int sz, const float tt = 0.025) \
	 : pk(0), level(1), Tlimit(tt), e(conn){
...
int main() {
	...
	while (scanf("%d", &n) != 0 && n) {
        ...
        Maxclique mc = Maxclique(vv, n);
		int answer, ans[N];
		mc.mcqdyn(ans, answer);
        printf("%d\n", answer);
    }
	return 0;
}
